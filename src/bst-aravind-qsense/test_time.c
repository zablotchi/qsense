...
test(void* thread) {
    ...
    while (final_stop == 0) {
        ...
        memory_owned[ID] = ltd.rcount + ssalloc_free_num[0] + ((SSALLOC_SIZE_SMALL - alloc_next[0])/sizeof(DS_NODE));
...
void print_statistics(size_t duration, int num_periods) {
    ...
    printf(" %7.1f%12.0f%10.3f%8lu%10lu%8lu%8lu", num_periods * duration/1000.0, throughput, throughput/1e6, allocate_fail, process_callbacks, total_memory_owned, total_rcount);
    // for (t = 0; t < num_threads; t++) {
